...
import { thumb } from '@/lib/basePath';
  ...
  return (
    <div className="sheet sheet--index">
      ...
      <div className="index">
        ...
        >
          {item.image ? (
            <Image src={thumb(item.image)} alt="" width={200} height={200} className="index__photo" />
          ) : (
          ...
          {item.children.map((c: Child, i) => (
            ...
            >
              <Link href={`/items/${item.slug}/${c.slug}`} className="entry">
                {c.image ? (
                  <Image src={thumb(c.image)} alt="" width={120} height={120} className="entry__photo" />
                ) : (
